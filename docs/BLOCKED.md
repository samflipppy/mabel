...
|---|---|---|---|---|
| 16 | Telnyx Call Control application | A Call Control app pointed at `/webhooks/telnyx/call`, and two test calls (one answered, one cancelled while ringing) | The missed-call text back | Built on four assumptions about the event names and payload shape, written up as T-1 to T-4 in `docs/telnyx_notes.md` and marked at each point in `webhooks/telnyx_calls.py` that relies on them. The handler fails closed: a missed call must be *positively* identified, so an unrecognised payload sends nothing rather than texting every caller. Verifying takes about ten minutes — the handler logs every event type and payload key it receives at INFO. |
| 15 | Telnyx 10DLC — **second campaign**, for customer-facing traffic | A registered A2P campaign whose use case covers messages to members of the public, separate from the owner-alert campaign in #4 | Every `customer_*` notification kind: call confirmations, emergency acknowledgements, review requests | `tenants.customer_sms_enabled` defaults to **false** and the settings screen says why. Messages sent under an unregistered campaign are accepted by the API and dropped by the carrier with no error the sender can see, so the default protects a tenant from discovering this through a customer complaint. Delivery receipts (`/webhooks/telnyx/status`) are the only place it becomes visible. |
| 1 | Supabase | `DATABASE_URL` (project Postgres, `mabel_app` role), `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY` | Every table, RLS, Auth, and Storage | Migrations can't be applied to a real project. Portal login. **The schema and RLS themselves are verified** — the isolation and end-to-end suites run against a scratch Postgres 16 (`docker run postgres:16`, `TEST_DATABASE_URL`), and 137 isolation tests plus 14 end-to-end tests pass against it. |
...
| S6 | `mabel_app` was granted table privileges but never `USAGE ON SCHEMA public`, and `mabel_admin` — which owns the SECURITY DEFINER resolution functions — had neither schema usage nor SELECT on the tables they read. Postgres reports a table in a schema you cannot USE as though it does not exist | RLS/grants block | Both granted explicitly in revision 0001. A brand new database grants USAGE on `public` to PUBLIC, so this was invisible until the isolation suite recreated the schema |
